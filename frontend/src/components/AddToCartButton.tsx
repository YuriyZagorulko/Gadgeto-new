...
import { useCartStore, useIsInCart, useCartSyncing, useCartInitialized } from '@/lib/cart-store';
...
export default function AddToCartButton({ product, className = '', compact = false }: AddToCartButtonProps) {
  ...
  const syncing = useCartSyncing();
  const initialized = useCartInitialized();
  ...
  // Before cart is initialized OR while syncing with backend — show a neutral
  // loading state so the button does NOT flicker between "Buy" and "In cart".
  if (!initialized || (syncing && !isInCart)) {
    if (compact) {
      return (
        <button
          disabled
          className={`p-2 rounded-lg bg-gray-200 text-gray-400 cursor-wait ${className}`}
    ...
    return (
      <button
        disabled
        className={`w-full text-lg py-3 rounded-lg font-medium bg-gray-200 text-gray-400 cursor-wait flex items-center justify-center gap-2 ${className}`}
      >
        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 animate-spin" fill="none" viewBox="0 0 24 24">
          <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
          <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
