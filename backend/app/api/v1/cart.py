...
router = APIRouter()
from app.core.db_connect import get_connection_dep
...
@router.get("/cart")
def get_cart(session_token: str = "", _db: tuple = Depends(get_connection_dep)):
    conn, cur = _db
...
def add_to_cart(req: CartItemRequest, session_token: str = "", _db: tuple = Depends(get_connection_dep)):
    conn, cur = _db
    cart_id, token = get_or_create_cart(cur, session_token)
...
def update_cart_item(item_id: int, req: CartItemUpdate, _db: tuple = Depends(get_connection_dep)):
    conn, cur = _db
    if req.qty <= 0:
...
def remove_cart_item(item_id: int, _db: tuple = Depends(get_connection_dep)):
    conn, cur = _db
    cur.execute("DELETE FROM cart_items WHERE id=%s", (item_id,))
    return {"ok": True}
