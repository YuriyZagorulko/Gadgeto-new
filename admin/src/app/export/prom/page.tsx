...
export default function PromOverviewPage() {
  ...
  const load = () => {
    ...
    Promise.all([
      api.get<Stats>('/export/channels/prom/stats'),
      api.get<{ items: TaxonomyStats }>('/export/channels/prom/taxonomy'),
    ])
  ...
  const loadRunStatus = () => {
    api.get<any>('/export/channels/prom/taxonomy/status')
      .then((d) => {
  ...
  const handleRefreshTaxonomy = async () => {
    ...
    try {
      const res = await api.post<{ run_id: number }>('/export/channels/prom/taxonomy/refresh');
  ...
  if (!stats) return <ErrorState message="Немає даних про канал Prom.ua" />;
  ...
  return (
    <div>
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-4 mb-6">
        <h3 className="text-sm font-medium text-gray-500 uppercase tracking-wide mb-2">Таксономія Prom.ua</h3>
        {taxData ? (
        ...
        {isRunActive && runStatus && (
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 mb-3">
            ...
            {runStatus.run_id && (
              <div className="mt-2">
                <Link href={`/export/prom/taxonomy/runs/${runStatus.run_id}`} className="text-xs text-blue-600 hover:underline">
                  Переглянути звіт →
                </Link>
              </div>
            )}
        ...
        {runStatus && !isRunActive && runStatus.status !== 'never' && (
          <div className="flex items-center gap-2 text-sm text-gray-600 mb-3">
            ...
            {runStatus.run_id && (
              <Link href={`/export/prom/taxonomy/runs/${runStatus.run_id}`} className="text-sm text-blue-600 hover:underline ml-2">
                Звіт →
              </Link>
            )}
          </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <a href="/export/prom/taxonomy" className="text-sm text-blue-600 hover:underline">
            Детальніше →
          </a>
        </div>
        {taxResult && (
        ...
        {!taxResult && taxData && taxData.categories === 0 && (
          <p className="text-xs text-gray-400 mt-1">
            Для завантаження таксономії потрібні облікові дані API Prom.ua (будуть додані у наступних фазах).
          </p>
        )}
      </div>
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <a
          href="/export/prom/taxonomy"
          className="inline-flex items-center justify-center rounded-md bg-white border border-gray-300 px-3 py-1.5 text-sm text-gray-900 hover:bg-gray-50"
        >
          Таксономия
        </a>
        <a
          href="/export/prom/products"
          className="inline-flex items-center justify-center rounded-md bg-white border border-gray-300 px-3 py-1.5 text-sm text-gray-900 hover:bg-gray-50"
        >
          Переглянути товари
        </a>
