...
type TaxonomyStats = { categories: number; attributes: number; main_filters: number; values: number };
type RunStatus = {
  run_id: number | null;
  status: string;
...
function HistoryTable({ refreshTrigger }: { refreshTrigger: number | null }) {
  ...
  const load = useCallback(() => {
    setLoading(true);
    api.get<{ items: RunHistoryRow[]; total: number }>('/export/channels/prom/taxonomy/runs?per_page=50')
      .then((d) => {
  ...
  const openDetail = async (run: RunHistoryRow) => {
    ...
    try {
      const d = await api.get<RunStatus>(`/export/channels/prom/taxonomy/runs/${run.id}`);
            ...
            <tr key={r.id} className="hover:bg-gray-50">
              ...
              <Td>
                <div className="flex gap-2">
                  ...
                  <Link href={`/export/prom/taxonomy/runs/${r.id}`} className="text-xs text-blue-600 hover:underline">Звіт</Link>
                </div>
              </Td>
            </tr>
          );
...
function useTaxonomyStatusPoll() {
  ...
  const load = useCallback(() => {
    api.get<RunStatus>('/export/channels/prom/taxonomy/status')
      .then((d) => {
...
export default function PromTaxonomyPage() {
  ...
  const catList = useLocalList<ExtCatRow>(
    '/export/channels/prom/taxonomy/categories' + qs({ page, per_page: perPage, q: appliedQ || undefined }),
    [page, perPage, appliedQ]);
  const attrList = useLocalList<ExtAttrRow>(
    '/export/channels/prom/taxonomy/attributes' + qs({ page, per_page: perPage, q: appliedQ || undefined }),
    [page, perPage, appliedQ]);
  const valList = useLocalList<ExtValRow>(
    '/export/channels/prom/taxonomy/values' + qs({ page, per_page: perPage, q: appliedQ || undefined }),
    [page, perPage, appliedQ]);
  ...
  const handleRefresh = async () => {
    try { await api.post('/export/channels/prom/taxonomy/refresh', {}); reloadStatus(); }
  ...
  return (
    <div>
      <PageHeader title="Таксономия Prom.ua" actions={
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <StatCard label="Основних фільтрів (main)" value={tax.main_filters} />
      ...
      {isRunning && (
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
          <div className="flex items-center gap-2 mb-2">
            <Spinner size="sm" />
            <span className="font-medium">Оновлення таксономії Prom.ua</span>
          </div>
          <div className="space-y-2 text-sm">
            ...
            <div className="mt-2">
              <Link href={`/export/prom/taxonomy/runs/${status.run_id}`} className="text-xs text-blue-600 hover:underline">
                Переглянути звіт →
              </Link>
            </div>
          </div>
      ...
      {['succeeded', 'partial', 'failed'].includes(status.status) && (
        <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 mb-6">
          ...
          <div className="mt-2">
            <Link href={`/export/prom/taxonomy/runs/${status.run_id}`} className="text-sm text-blue-600 hover:underline">
              Переглянути звіт →
            </Link>
          </div>
        </div>
      ...
      {tab === 'history' ? (
        ...
        <>
                    {tab === 'categories' && (
            <Table head={<><Th>Prom.ua ID</Th><Th>Назва</Th><Th>Батьківська</Th><Th>Шлях</Th><Th>Атриб.</Th></>}>
          ...
          {tab === 'attributes' && (
            <Table head={<><Th>Назва</Th><Th>Prom.ua ID</Th><Th>Категорія</Th><Th>Тип</Th><Th>Од.</Th><Th>Обов'язковий</Th></>}>
          ...
          {tab === 'values' && (
            <Table head={<><Th>Атрибут</Th><Th>Prom.ua ID</Th><Th>Значення</Th><Th>Категорія</Th></>}>
