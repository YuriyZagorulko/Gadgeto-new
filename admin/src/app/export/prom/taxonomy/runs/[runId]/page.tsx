...
export default function TaxonomyRunReportPage() {
  ...
  const load = useCallback(async () => {
    try {
      const d = await api.get<RunReport>(`/export/channels/prom/taxonomy/runs/${runId}`);
  ...
  return (
    <div>
      <PageHeader
        title={`Оновлення таксономії Prom.ua #${r.run_id}`}
        actions={
          <Link href="/export/prom/taxonomy" className="text-sm text-blue-600 hover:underline">
            ← До таксономії
          </Link>
        }
      />
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <StatCard label="Всього категорій Prom.ua" value={r.attributes.categories_total} tone="blue" />
      ...
      <div className="bg-gray-50 border border-gray-200 rounded-lg p-3 mb-6 text-xs text-gray-500">
        ...
        було завантажено атрибути. «Всього категорій Prom.ua» — загальна кількість категорій у таксономії.
        Якщо ці числа не збігаються, деякі категорії не мають визначених атрибутів у API Prom.ua.
      </div>
