...
export default function PromPricingTab() {
  ...
  const loadStatus = () => {
    setLoading(true);
    api.get<PricingStatus>('/pricing/prom/status')
      .then((d) => setStatus(d))
  ...
  const loadRules = () => {
    ...
    api.get<RulesResp>('/pricing/prom/rules' + qs(params))
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const res = await fetch('/api/pricing/prom/import', {
        method: 'POST',
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        <h3 className="text-base font-semibold mb-4">Прайс / комісія Prom.ua</h3>
        {status?.active ? (
          ...
          <p className="text-sm text-gray-500 py-2">Прайс Prom.ua не завантажено. Експорт використовує налаштування націнки зі сторінки «Експорт».</p>
        )}
      </div>

      <div className="bg-white rounded-lg border border-gray-200 p-6">
        <h3 className="text-base font-semibold mb-2">Оновити прайс Prom.ua</h3>
        <p className="text-xs text-gray-500 mb-4">Завантажте файл .xlsx із комісіями Prom.ua.</p>
          ...
          {rulesLoading ? <LoadingState label="..." /> : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="text-xs text-gray-500 uppercase bg-gray-50">
                  <tr>
                    <th className="p-2 text-left">Категорія Prom.ua</th>
