...
export default function ExportHistoryPage() {
  ...
  const load = useCallback(() => {
    ...
    api.get<ListResp>('/export/channels/prom/history' + qs(p))
      .then((d) => setData(d))
  ...
  const handleDelete = async () => {
    ...
    try {
      await api.delete('/export/channels/prom/history/' + confirmDel);
  ...
  const handleCancel = async (runId: number) => {
    ...
    try {
      const res = await api.post<{ detail: string }>('/export/channels/prom/export/' + runId + '/cancel');
  ...
  return (
    <div>
      <PageHeader title="Історія експортів Prom.ua" />
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <Link href="/export/prom/settings" className="ml-auto text-sm text-blue-600 hover:text-blue-800 font-medium">
      ...
      {loading ? (
        ...
        <EmptyState title="Немає експортів" hint="Експорт на Prom.ua ще не запускався." />
      ) : (
              ...
              {data.items.map((r) => {
                ...
                return (
                  <tr key={r.id} className={`hover:bg-gray-50 ${isActive ? 'bg-blue-50' : ''}`}>
                    <td className="p-2">
                      <Link href={`/export/prom/history/${r.id}`} className="text-blue-600 hover:text-blue-800 font-mono">
                        #{r.id}
                      </Link>
                    </td>
                    <td className="p-2">
      ...
      <ConfirmDialog
        ...
        message={`Видалити експорт #${confirmDel}? Товари та список Prom.ua не зміняться.`}
