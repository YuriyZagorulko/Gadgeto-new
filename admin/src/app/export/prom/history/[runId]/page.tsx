...
const MAPPING_PAGE = '/export/prom/mapping';
...
function SkippedIssues({ r }: { r: ProductResult }) {
  ...
  return (
    <div className="space-y-2">
      ...
      {catId && <div className="text-gray-400">Категорія Prom.ua: <span className="font-mono">{catId}</span></div>}
      {attrs.length > 0 && (
        <div>
    ...
    <div className="space-y-2">
      ...
      {mAttrs.length > 0 && (
        <div>
          ...
          <Link
            ...
            href={`/export/prom/mapping?tab=attributes&q=${q(mAttrs[0]?.attribute_name)}`}
          >
            Замапити атрибут →
          </Link>
        </div>
      ...
      {mVals.length > 0 && (
        <div>
          ...
          <Link
            className="inline-block mt-0.5 text-blue-600 hover:text-blue-800 underline"
            href={`/export/prom/mapping?tab=values&q=${q(mVals[0]?.attribute_name)}`}
          >
            Замапити значення →
          </Link>
        </div>
      ...
      {issues.external_category_id && (
        <div className="text-gray-400">Категорія Prom.ua: {issues.external_category_id}</div>
      )}
    </div>
  );
}
...
export default function ExportDetailPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    api.get<ExportDetail>(`/export/channels/prom/history/${runId}`)
      .then((d) => setReport(d))
  ...
  return (
    <div className="max-w-5xl space-y-6">
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <Link href="/export/prom/history" className="ml-auto text-sm text-blue-600 hover:text-blue-800 font-medium">← Назад до історії</Link>
      </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-5 space-y-4">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
          ...
          <div><div className="text-gray-500 text-xs">Тип</div><div className="font-medium">Експорт на Prom.ua</div></div>
        </div>
      </div>
