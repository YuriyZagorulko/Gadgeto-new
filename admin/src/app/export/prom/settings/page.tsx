...
import PromPricingTab from '../components/PromPricingTab';
...
export default function PromSettingsPage() {
  ...
  useEffect(() => {
    ...
    api.get<ProductsResp>('/export/channels/prom/products' + qs(params))
  ...
  const executeExport = async (ids?: number[]) => {
    ...
    try {
      ...
      const data = await api.post<{ run_id: number; status: string; total: number }>(
        '/export/channels/prom/export', body);
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        const d = await api.get<any>(`/export/channels/prom/export/status/${exportRunId}`);
        setExportStatus(d);
  ...
  const load = useCallback(() => {
    ...
    api.get<SettingsResp>('/export/channels/prom/settings')
      .then((d) => {
  ...
  const updateSetting = async (key: string, value: string) => {
    setSaving(true);
    try {
      await api.put('/export/channels/prom/settings', { key, value, is_secret: false });
  ...
  return (
    <div>
      <PageHeader title="Налаштування Prom.ua" />
        ...
        {exportRunId && (
          <div className="mt-4 p-4 bg-blue-50 border border-blue-100 rounded-lg">
            ...
            <div className="mt-2 flex gap-3">
              <Link href="/export/prom/history" className="text-xs text-blue-700 hover:text-blue-900 underline">
                Переглянути історію експортів →
              </Link>
              {finalExportRunId && (
                <Link href={`/export/prom/history/${finalExportRunId}`} className="text-xs text-blue-700 hover:text-blue-900 underline">
      ...
      {tab === 'general' && (
        <div className="space-y-6">
          <div className="bg-white rounded-lg border border-gray-200 p-6">
            ...
            <p className="text-xs text-gray-500 mb-4">Загальні налаштування каналу Prom.ua.</p>
      ...
      {tab === 'export' && (
        <div className="bg-white rounded-lg border border-gray-200 p-6">
          ...
          <p className="text-xs text-gray-500 mb-6">Ці налаштування застосовуються під час експорту товарів до Prom.ua.</p>
      ...
      <ConfirmModal
        ...
        message="Буде запущено експорт усіх доступних для експорту товарів у Prom.ua."
