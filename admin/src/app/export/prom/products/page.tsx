...
export default function PromExportPage() {
  ...
  const load = useCallback(() => {
    ...
    api.get<ProductsResp>('/export/channels/prom/products' + qs(filterParams))
  ...
  const runPreview = async () => {
    ...
    try {
      ...
      const data = await api.post<PreviewResp>('/export/channels/prom/export/preview', body);
      setPreview(data);
  ...
  return (
    <div>
      <PageHeader title="Експорт товарів у Prom.ua" />

      <div className="flex flex-wrap items-center gap-3 mb-4">
        <Button variant="primary" onClick={() => handleExportAll()}>Експортувати всі товари на Prom.ua</Button>
        <Button disabled={selectionCount === 0} variant="secondary" onClick={() => handleExportSelected()}>
          Експортувати вибрані товари на Prom.ua ({selectionCount})</Button>
      </div>

      <div className="flex flex-wrap gap-4 mb-4 text-xs text-gray-600">
        <span>Націнка Prom.ua: </span>
        <a href="/export/prom/settings" className="text-blue-600 hover:underline">Налаштування →</a>
      </div>
              ...
              rows.map((r) => {
                ...
                return (
                  <tr key={r.id} className="hover:bg-gray-50">
                    ...
                    <Td className="text-xs font-mono max-w-28 truncate"><span title={r.sku}>{r.sku || '—'}</span></Td>
                    <Td className="max-w-xs truncate"><span title={r.name}>{r.name || '—'}</span></Td>
