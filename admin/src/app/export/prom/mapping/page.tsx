...
import PromCategoryMappingFilterPanel from '@/components/mapping/PromCategoryMappingFilters';
import type { PromCategoryMappingFilters as PromCategoryMappingFilterValues } from '@/components/mapping/PromCategoryMappingFilters';
import PromAttributeMappingFilterPanel from '@/components/mapping/PromAttributeMappingFilters';
import type { PromAttributeMappingFilters as PromAttributeMappingFilterValues } from '@/components/mapping/PromAttributeMappingFilters';
import PromValueMappingFilterPanel from '@/components/mapping/PromValueMappingFilters';
import type { PromValueMappingFilters as PromValueMappingFilterValues } from '@/components/mapping/PromValueMappingFilters';
...
function PromPicker<T extends { external_id: string; name?: string; value?: string }>({
  endpoint,
  label,
...
export default function PromMappingPage() {
  return (
...
function PromMappingInner() {
  ...
  // Prom.ua entities chosen via pickers
...
const [fExtCatChildren, setFExtCatChildren] = useState(0);
  ...
  const loadCoverage = useCallback(() => {
    api.get<Coverage>('/export/channels/prom/mapping-coverage')
  ...
  useEffect(() => {
    ...
    api.get<{ items: ExtCatOpt[] }>('/export/channels/prom/pickers/external-categories?per_page=200')
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await api.delete(`/export/channels/prom/mappings/${tab}/${id}`);
  ...
  const saveMapping = async (body: any) => {
    setSaving(true);
    try {
      await api.post(`/export/channels/prom/mappings/${tab}`, body);
  ...
  const updateMapping = async (id: number, body: any) => {
    setSaving(true);
    try {
      await api.put(`/export/channels/prom/mappings/${tab}/${id}`, body);
  ...
  return (
    <div>
      <PageHeader title="Маппінг Prom.ua" actions={tab !== 'coverage' && (
...
function renderCategories(rows: any[], openEdit: (r: any) => void) {
  return (
    <Table head={<><Th>Внутрішня категорія</Th><Th>→ Prom.ua</Th><Th>Тип</Th><Th>Статус</Th><Th>Confidence</Th><Th>Джерело</Th><Th className="w-24">Дії</Th></>}>
      {rows.length === 0 ? (
...
// MainFilterBadge: displays whether this attribute is a Prom.ua main filter (filter_type="main").
// IMPORTANT: "Main filter" does NOT mean "required attribute". Prom.ua has no required attributes.
...
function renderAttributes(rows: any[], openEdit: (r: any) => void) {
  return (
    <Table head={<><Th>Внутрішній атрибут</Th><Th>Prom.ua категорія</Th><Th>→ Prom.ua атрибут</Th><Th>Основний фільтр</Th><Th>Статус</Th><Th>Confidence</Th><Th className="w-24">Дії</Th></>}>
      {rows.length === 0 ? (
        <tr><td colSpan={7} className="p-6 text-center text-gray-400">Немає відповідностей</td></tr>
      ) : rows.map((r: any) => {
...
function renderValues(rows: any[], openEdit: (r: any) => void, unmappedMode = false) {
  const head = unmappedMode
    ? <><Th>Внутрішнє значення</Th><Th>Атрибут</Th><Th>Prom.ua атрибут</Th><Th className="text-right">Товарів</Th><Th>Статус</Th><Th className="w-24">Дії</Th></>
    : <><Th>Внутрішнє значення</Th><Th>Атрибут</Th><Th>Prom.ua атрибут</Th><Th>→ Prom.ua значення</Th><Th>Статус</Th><Th className="w-24">Дії</Th></>;
  const colSpan = unmappedMode ? 6 : 6;
  return (
    <Table head={head}>
      {rows.length === 0 ? (
        <tr><td colSpan={colSpan} className="p-6 text-center text-gray-400">
          {unmappedMode ? 'Немає незіставлених значень. Усі знайдені значення вже мають відповідності Prom.ua.' : 'Немає відповідностей'}
        </td></tr>
      ) : rows.map((r: any) => {
        const sb = statusBadge[r.status || 'unmapped'] || { tone: 'gray' as const, label: r.status || 'unmapped' };
        return (
...
function CategoriesMappingTab({ openEdit, openDelete }: {
  ...
  const [appliedFilters, setAppliedFilters] = useState<PromCategoryMappingFilterValues>({
  ...
  const load = useCallback(() => {
    ...
    api.get<ListResp<any>>('/export/channels/prom/mappings/categories' + qs(params))
  ...
  const handleApply = (filters: PromCategoryMappingFilterValues) => {
    setAppliedFilters(filters);
    setPage(1);
  };
  return (<>
    <PromCategoryMappingFilterPanel onApply={handleApply} />
...
function AttributesMappingTab({ openEdit, openDelete }: {
  ...
  const [appliedFilters, setAppliedFilters] = useState<PromAttributeMappingFilterValues>({
  ...
  const load = useCallback(() => {
    ...
    api.get<ListResp<any>>('/export/channels/prom/mappings/attributes' + qs(params))
      .then((d) => { setRows(d.items); setTotal(d.total); })
      .catch((e: any) => setError(e.message || 'Не вдалось завантажити'))
      .finally(() => setLoading(false));
  }, [page, perPage, appliedFilters]);
  ...
  useEffect(() => { setPage(1); }, [appliedFilters]);
  const pages = Math.max(1, Math.ceil(total / perPage));
  const handleApply = (filters: PromAttributeMappingFilterValues) => {
    setAppliedFilters(filters);
    setPage(1);
  };
  return (<>
    <PromAttributeMappingFilterPanel onApply={handleApply} />
    {loading ? <LoadingState /> : error ? <ErrorState message={error} onRetry={load} /> : (<>
...
function ValuesMappingTab({ openEdit, openDelete }: {
  ...
  const [appliedFilters, setAppliedFilters] = useState<PromValueMappingFilterValues>({
    internalAttrIds: '', externalAttrIds: '', externalCategoryIds: '',
  ...
  const load = useCallback(() => {
    ...
    api.get<ListResp<any>>('/export/channels/prom/mappings/values' + qs(params))
      .then((d) => { setRows(d.items); setTotal(d.total); })
      .catch((e: any) => setError(e.message || 'Не вдалось завантажити'))
      .finally(() => setLoading(false));
  }, [page, perPage, appliedFilters]);
  ...
  useEffect(() => { setPage(1); }, [appliedFilters]);
  const pages = Math.max(1, Math.ceil(total / perPage));
  const handleApply = (filters: PromValueMappingFilterValues) => {
    setAppliedFilters(filters);
    setPage(1);
  };
  return (<>
    <PromValueMappingFilterPanel onApply={handleApply} />
    {loading ? <LoadingState /> : error ? <ErrorState message={error} onRetry={load} /> : (<>
  ...
  return (
    <Modal open={open} title={editing ? 'Редагування маппінгу категорії' : 'Створення маппінгу категорії'} onClose={onCancel} wide>
      <div className="space-y-4">
        ...
        {/* Prom.ua category picker */}
        <div>
          <PromPicker<ExtCatOpt>
            endpoint="/export/channels/prom/pickers/external-categories"
            label="Категорія Prom.ua"
            value={fExtCatId}
        ...
        {/* Selected entity card */}
        <SelectedCard label="ОБРАНА КАТЕГОРІЯ PROM.UA" name={fExtCatName} id={fExtCatId} />

        {isParentWarning && (
          <div className="bg-yellow-50 border border-yellow-300 rounded-md p-3 text-sm text-yellow-800">
            <span className="font-medium">⚠ Увага:</span> Ця категорія Prom.ua є батьківською ({fExtCatChildren} дочірніх) та не має власних характеристик ({fExtCatAttrs} атр.).
  ...
  return (
    <Modal open={open} title={editing ? 'Редагування маппінгу атрибуту' : 'Створення маппінгу атрибуту'} onClose={onCancel} wide>
      <div className="space-y-4">
        ...
        {/* Prom.ua category context */}
        <PromPicker<ExtCatOpt>
          endpoint="/export/channels/prom/pickers/external-categories"
          label="Контекст — категорія Prom.ua (необов'язково для глобальних атрибутів)"
          value={fExtCatId}
          onChange={(item) => onChangeCategory(item as ExtCatOpt | null)}
          displayName={(item) => item.name}
        />
        <SelectedCard label="КАТЕГОРІЯ PROM.UA" name={fExtCatName} id={fExtCatId} />

        {/* Prom.ua attribute — filtered by category */}
        <PromPicker<ExtAttrOpt>
          endpoint="/export/channels/prom/pickers/external-attributes"
          label="Атрибут Prom.ua"
  ...
  return (
    <Modal open={open} title={editing ? 'Редагування маппінгу значення' : 'Створення маппінгу значення'} onClose={onCancel} wide>
      <div className="space-y-4">
        ...
        {/* Prom.ua attribute — filtered by category when chosen */}
        <PromPicker<ExtAttrOpt>
          endpoint="/export/channels/prom/pickers/external-attributes"
          label="Атрибут Prom.ua"
          extraParams={extAttrExtra}
        ...
        />
        <SelectedCard label="АТРИБУТ PROM.UA" name={fExtAttrName} id={fExtAttrId} />

        {/* Prom.ua value — scoped when category + attribute are chosen;
            unscoped list is 700k+ rows, so it is search-driven */}
        <PromPicker<ExtValOpt>
          endpoint="/export/channels/prom/pickers/external-values"
          label="Значення Prom.ua"
          ...
          emptyHint={
            fExtCatId && fExtAttrId
              ? `У атрибута «${fExtAttrName}» немає словника значень Prom.ua — це поле вільного введення (наприклад, EAN)`
              : 'Немає значень у словнику Prom.ua для обраного фільтра'
          }
