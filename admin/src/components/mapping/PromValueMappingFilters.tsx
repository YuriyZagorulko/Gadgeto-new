...
export interface PromValueMappingFilters {
  internalAttrIds: string;       // comma-separated internal attribute IDs
  externalAttrIds: string;       // comma-separated external (Prom.ua) attribute IDs
...
interface Props {
  onApply: (filters: PromValueMappingFilters) => void;
}

export default function PromValueMappingFilterPanel({ onApply }: Props) {
    ...
    <div className="flex flex-wrap items-end gap-3 mb-4">
      ...
      <div className="w-72">
        <EntityMultiSelect
          ...
          idKey="external_id"
          nameKey="name"
          placeholder="Введіть для пошуку атрибутів Prom.ua..."
        />
      </div>
      <div className="w-64">
        <EntityMultiSelect
          endpoint="/export/channels/prom/pickers/external-categories"
          label="Контекст — категорія Prom.ua"
      ...
      <div>
        <label className="block text-xs text-gray-500 mb-1">Значення Prom.ua</label>
        <Input value={externalValueQ} onChange={(e) => setExternalValueQ(e.target.value)}
          placeholder="Пошук значення Prom.ua..." className="w-48"
          onKeyDown={(e) => { if (e.key === 'Enter') apply(); }} />
      </div>
      <div>
